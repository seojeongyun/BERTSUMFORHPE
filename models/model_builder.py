...
def build_optim(args, model, checkpoint):
    ...
    if checkpoint is not None and 'optim' in checkpoint and checkpoint['optim'] is not None:
        ckpt_optim = checkpoint['optim']
        optim = ckpt_optim[0] if isinstance(ckpt_optim, (list, tuple)) else ckpt_optim

        saved_state = None
        if hasattr(optim, "optimizer") and optim.optimizer is not None:
            saved_state = optim.optimizer.state_dict()
        saved_state['param_groups'][0]['lr'] = args.lr
        print(saved_state['param_groups'][0]['lr'])
        optim.set_parameters(model.named_parameters())

        if saved_state is not None:
            optim.optimizer.load_state_dict(saved_state)

        if args.visible_gpus != '-1':
            for state in optim.optimizer.state.values():
                for k, v in state.items():
                    if torch.is_tensor(v):
                        if k in ['step', 'step_t', 'state_steps']:
                            state[k] = v.cpu()
                        else:
                            state[k] = v.cuda()
...
class Bert(nn.Module):
    def __init__(self, large, temp_dir, finetune=False):
        ...
        if(large):
            ...
            self.model, _ = get_pytorch_kobert_model(ctx='cuda:0', cachedir=temp_dir)
            # BERT = (embedding, encoder) layers -> remove embedding block
                # instead, add positional embedidng
...
class ExtSummarizer(nn.Module):
    def __init__(self, args, device, checkpoint):
        ...
        self.args = args
        self.device = device
        self.bert = Bert(args.large, args.temp_dir, args.finetune_bert).to(device)
